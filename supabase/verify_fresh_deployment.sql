...
begin
  ...
  if not exists (select 1 from pg_trigger where tgname='on_auth_user_created' and not tgisinternal) then raise exception 'Auth profile trigger is missing'; end if;
  if exists (select 1 from (values ('profiles'),('student_profiles'),('employee_profiles'),('resume_analyses'),('trust_cards'),('referral_requests'),('referral_status_history'),('proof_entries'),('notifications')) x(name) join pg_class c on c.relname=x.name join pg_namespace n on n.oid=c.relnamespace and n.nspname='public' where not c.relrowsecurity) then raise exception 'A required table does not have RLS enabled'; end if;
