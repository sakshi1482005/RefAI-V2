...
assert.doesNotMatch(notifications, /if \(open && loaded\) void load\(\)/, 'opening notifications must reuse the initial authenticated fetch instead of refetching unchanged data')
assert.match(notifications, /AbortController/, 'notification reads must abort when their component is discarded')
...
assert.match(notifications, /useAuthSession/, 'notifications must rely on the authenticated session only')
assert.match(notifications, /notifications\/clear-all/, 'notifications must clear through the authenticated backend route')
assert.match(notifications, /setItems\(\[\]\)/, 'clearing notifications must update the visible list immediately')
assert.match(notifications, /clearing \|\| !items\.length/, 'notification clearing must prevent duplicate requests')
