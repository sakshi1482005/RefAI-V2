import { FileText, MessageSquareText, Paperclip, RefreshCw, ShieldCheck, Sparkles } from 'lucide-react'
...
import EmployeeDecisionPanel from './EmployeeDecisionPanel'
...
export default function AuthenticatedCandidateReview({ requestId }: { requestId: string }) {
  ...
  const summarizeCandidate = async (refresh = false) => {
    if (copilotLoading) return
    setCopilotLoading(true)
    setCopilotError(null)
    try {
      const { data } = await api.post<EmployeeReviewCopilot>(`/referral/employee/requests/${requestId}/copilot${refresh ? '?refresh=true' : ''}`)
      setCopilot(data)
    } catch (error) {
    ...
    <div className="mt-5 grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_19rem]">
      <div className="min-w-0 space-y-5">
        <Card className="border-blue-200 p-5 sm:p-6">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><Sparkles className="size-5 text-blue-700" aria-hidden="true" /><h2 className="text-lg font-semibold">Employee Review Copilot</h2><Badge tone="info">Advisory AI</Badge></div><p className="mt-2 text-sm leading-6 text-slate-600">Evidence summary only. The employee makes every approval, decline, or information-request decision.</p></div><div className="flex flex-wrap gap-2"><PrimaryButton onClick={() => { void summarizeCandidate() }} loading={copilotLoading} disabled={copilotLoading} disabledReason="Preparing the advisory summary"><Sparkles className="mr-2 size-4" />Summarize Candidate in 30 Seconds</PrimaryButton>{copilot ? <SecondaryButton onClick={() => { void summarizeCandidate(true) }} disabled={copilotLoading} disabledReason="Preparing the advisory summary"><RefreshCw className="mr-2 size-4" />Regenerate</SecondaryButton> : null}</div></div>
        ...
        {detail.studentResponse ? <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm"><div className="flex items-center gap-2"><MessageSquareText className="size-5 text-emerald-700" /><h2 className="text-base font-semibold text-emerald-950">Student response to clarification</h2></div>{detail.moreInformationQuestion ? <p className="mt-3 text-xs leading-5 text-emerald-900"><span className="font-semibold">Your question:</span> {detail.moreInformationQuestion}</p> : null}<p className="mt-3 text-sm leading-6 text-emerald-950">{detail.studentResponse}</p>{detail.studentResponseProofEntries.length ? <div className="mt-3 border-t border-emerald-200 pt-3"><p className="flex items-center gap-2 text-xs font-semibold text-emerald-900"><Paperclip className="size-3.5" />Attached Proof Vault evidence</p>{detail.studentResponseProofEntries.map((proof) => <p key={proof.id} className="mt-1 text-xs text-emerald-900">• {proof.title}</p>)}</div> : null}</section> : null}
      ...
      <aside className="self-start xl:sticky xl:top-24">
        <Card className="p-5">
          ...
          <EmployeeDecisionPanel requestId={requestId} disabled={!workflow.canMakeDecision} />
