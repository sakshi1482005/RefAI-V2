...
export default function AIOpportunityRecommendations() {
  ...
  useEffect(() => {
    ...
    api.get<AIApplyAllowance>('/ai-apply/allowance').then(({ data }) => {
      if (active) { setAllowance(data); setAllowanceError(false) }
    }).catch((cause) => {
      if (active) { setAllowanceError(true); console.warn('[RefAI allowance unavailable]', cause) }
