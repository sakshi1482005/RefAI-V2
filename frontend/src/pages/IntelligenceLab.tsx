...
export default function IntelligenceLab() {
  ...
  if ((analysisResource.error || comparison.error) && !data) return <PageShell compact eyebrow="Capstone explainability" title="RefAI Intelligence Lab" description="The academic comparison could not be loaded from your saved analysis."><InlineFeedback tone="error">{friendlyErrorMessage(analysisResource.error || comparison.error, 'Please retry the comparison.')}</InlineFeedback><div className="mt-5 flex gap-2"><PrimaryButton onClick={() => { analysisResource.retry(); comparison.retry() }}>Retry</PrimaryButton><SecondaryButton onClick={() => navigate('/dashboard/trust-card')}>Back to Trust Card</SecondaryButton></div></PageShell>
  ...
  return <PageShell compact eyebrow="Capstone explainability & evaluation" title="RefAI Intelligence Lab" description={`Read-only algorithm comparison for the current ${data.targetRole || 'candidate'} profile. It explains saved deterministic outputs; it does not predict hiring, referrals, or model accuracy.`} action={<SecondaryButton onClick={() => navigate('/dashboard/trust-card')}><ArrowLeft className="mr-2 size-4" />Back to Trust Card</SecondaryButton>}>
    {comparison.error ? <InlineFeedback tone="info">Saved model comparison is shown. RefAI could not refresh it just now. <button type="button" className="font-semibold underline" onClick={comparison.retry}>Retry</button></InlineFeedback> : null}
