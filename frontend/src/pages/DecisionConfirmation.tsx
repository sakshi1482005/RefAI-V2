...
export default function DecisionConfirmation() {
  ...
  const resource = useEmployeeRequestResource(
    requestId ? `/referral/employee/requests/${requestId}` : null,
    parseEmployeeRequestDetail,
  )
  let record = immediateRecord

  const persisted = submissionResult ?? resource.data
  if (persisted && ['approved', 'declined', 'more_info_requested', 'referred'].includes(persisted.status)) {
    record = {
  ...
  return (
    ...
    >
      {record ? <div className="mx-auto max-w-3xl space-y-6">
        <InlineFeedback tone="info">This decision was saved to the assigned referral request.</InlineFeedback>
